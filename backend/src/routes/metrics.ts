 ...
 *
 * GET /metrics/series
 *   Time-bucketed series for one (kind, subject?, metric) triple.
 *   Query: kind, subject? metric, rangeMinutes (default 60),
 *          bucketSeconds (default 30).
 *
 * GET /metrics/latest
 *   Most recent value per (kind, subject, metric). Used for KPI cards.
 *
 * GET /metrics/containers
 *   Latest CPU/MEM per Docker container, sorted by cpu_pct desc.
 */
...
router.get("/series", async (req: Request, res: Response) => {
  ...
  try {
    const subjectClause = subject === null
      ? `AND subject IS NULL`
      : `AND subject = $5`;
    const params: unknown[] = [kind, metric, rangeMinutes, bucketSeconds];
    ...
    res.json({
      kind, subject, metric, rangeMinutes, bucketSeconds,
